...
export function calculateAllRisks(tms:TmMaster[],history:ReplacementHistory[],sm:SeverityMaster[],s:RiskSettings):RiskScore[]{
 ...
 const failureCounts=new Map<string,number>();failures.forEach(r=>{if(r.removedSerialNo)failureCounts.set(r.removedSerialNo,(failureCounts.get(r.removedSerialNo)??0)+1);});const maxFailureCount=Math.max(...failureCounts.values(),0);
