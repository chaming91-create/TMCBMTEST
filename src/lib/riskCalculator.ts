...
const validSerial=(v:unknown)=>{const x=String(v??'').trim();return !!x&&x!=='-'&&x!=='모름';};
const severity=(r:ReplacementHistory,sm:SeverityMaster[])=>typeof r.severityScore==='number'&&Number.isFinite(r.severityScore)&&r.severityScore>0?r.severityScore:sm.find(v=>v.isActive&&v.failureType===r.failureType)?.severityScore??0;
const uncertaintyReasons=(tm:TmMaster,history:ReplacementHistory[])=>{const reasons:string[]=[];if(unknown(tm.manufacturer))reasons.push('제조사 모름');if(!tm.manufactureYear)reasons.push('제작년도 모름');if(unknown(tm.currentStatus))reasons.push('현재상태 모름');if(!tm.isSpare&&tm.sourceType!=='history_only'){if(unknown(tm.currentTrain))reasons.push('편성 모름');if(unknown(tm.currentPosition))reasons.push('위치 모름');}history.filter(r=>r.removedSerialNo===tm.serialNo).forEach(r=>{if(unknown(r.failureType))reasons.push('고장유형 모름');if(r.severityScore==null)reasons.push('고장심각도 모름');if(!r.replacementDate)reasons.push('교체일자 모름');});return Array.from(new Set(reasons));};
export function calculateAllRisks(tms:TmMaster[],history:ReplacementHistory[],sm:SeverityMaster[],s:RiskSettings):RiskScore[]{
 if(!tms.length)return[];const failures=history.filter(r=>isFailure(r,s));const attributableFailures=failures.filter(r=>validSerial(r.removedSerialNo));const globalRate=failures.length/tms.length;const groups=new Map<string,{tmCount:number;failureCount:number}>();
 ...
 attributableFailures.forEach(r=>{const tm=tms.find(t=>t.serialNo===r.removedSerialNo);if(tm){const g=groups.get(keyOf(tm));if(g)g.failureCount++;}});
 const failureCounts=new Map<string,number>();attributableFailures.forEach(r=>{if(r.removedSerialNo)failureCounts.set(r.removedSerialNo,(failureCounts.get(r.removedSerialNo)??0)+1);});const maxFailureCount=Math.max(...failureCounts.values(),0);
 const totals=new Map<string,number>();attributableFailures.forEach(r=>{if(r.removedSerialNo)totals.set(r.removedSerialNo,(totals.get(r.removedSerialNo)??0)+severity(r,sm));});const maxT=Math.max(...totals.values(),0);const m=calculateMkbfScore(s.mkbf),now=new Date().toISOString();
 return tms.map(tm=>{const AScore=calculateAgeScore(tm.manufactureYear,s),FScore=maxFailureCount?round(100*(failureCounts.get(tm.serialNo)??0)/maxFailureCount):0,TScore=maxT?round(100*(totals.get(tm.serialNo)??0)/maxT):0,riskScore=round(s.weights.A*AScore+s.weights.F*FScore+s.weights.T*TScore+s.weights.M*m),reasons=uncertaintyReasons(tm,history);return{serialNo:tm.serialNo,AScore,FScore,TScore,MScore:m,riskScore,riskGrade:getRiskGrade(riskScore,s),isUncertain:reasons.length>0,uncertaintyReasons:reasons,calculatedAt:now,formulaVersion:s.formulaVersion};});
}
